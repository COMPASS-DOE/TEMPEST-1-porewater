...
npoc_wflags_metadata <-  npoc_flags %>%
  mutate(sample_name = stringr::str_replace(sample_name,"_DOC",""), #dropping the DOC from the sample name
         sample_name = stringr::str_replace(sample_name,"HR6","HR7"), #fixing the incorrect sample name
  ...
  mutate(date = case_when(is.na(date)~ "20220622", #fixing the NAs for the estuary sampling 
                          TRUE ~ date),
         time = case_when(Timepoint == "HR4" ~ "110000" , #fixing the NAs for the estuary sampling 
                          Timepoint == "HR8" ~ "150000", #fixing the NAs for the estuary sampling 
                          TRUE ~ time)) %>%
