...
sample_key <- readxl::read_excel("~/GitHub/TEMPEST-1-porewater/data_do_not_commit/porewaterinventory.xlsx", sheet = "Porewater - Individual", skip = 3) %>%
...
pH <- pH_raw %>%
  ...
  select(Site, plot, grid, Cond, Cond_units, collection_temp_C, datetime, Date_YYYYMMDD, Time_24hr, tz, flag, Notes)
```
...
porewater_pH_2023 <- porewater_pH %>% filter(datetime < year2_stop & datetime > year2_start) %>%
  select(Site, plot, grid, Cond, Cond_units, collection_temp_C, datetime, Date_YYYYMMDD, Time_24hr, tz, evacuation_date, flag)
...
porewater_pH_2025 <- porewater_pH %>% filter(datetime < year4_stop & datetime > year4_start) %>%
 ...
 write_csv(porewater_pH_2023, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_spc_2023_L1.csv")
 write_csv(porewater_pH_2024, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_spc_2024_L1.csv")
 write_csv(porewater_pH_2025, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_spc_2025_L1.csv")
...
```{r tempest porewater spc all same units}

porewater_spc_all <- porewater_pH_2024 %>%
  full_join(porewater_pH_2025) %>%
  mutate(spc_uscm = case_when(Cond_units == "mS/cm" ~ as.numeric(Cond) * 1000,
                               TRUE ~ as.numeric(Cond)) ) %>%
  select(Site, plot, grid, spc_uscm, datetime, Date_YYYYMMDD, Time_24hr, tz, evacuation_date)

 write_csv(porewater_spc_all, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_spc_2024-2025.csv")
 
 porewater_spc_control <- porewater_spc_all %>%
   filter(plot == "Control")
 
 min( porewater_spc_control$spc_uscm)
 
 max( porewater_spc_control$spc_uscm)
 
  write_csv(porewater_spc_control, "~/GitHub/TEMPEST-1-porewater/processed data/for_google_drive/TMP_PW_spc_control_2024-2025.csv")
