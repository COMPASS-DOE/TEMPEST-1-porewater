...
read_data <- function(name){
  ...
  clean_names() %>% 
  filter(str_detect(research_name, "soil_vwc") | str_detect(research_name, "soil_EC") | str_detect(research_name, "wx_rain24")) %>% #pull the teros vwc, EC, and the rain data
...
Rain actuall only lives in GCW-W: https://github.com/COMPASS-DOE/data-workflows/issues/158
...
p <- teros_qc1 %>%
  filter(variable == "vwc") %>%
  ggplot(aes(datetime_est, value, linetype= depth, color = location)) + 
  geom_point() + 
...
```{r scrub the sketch}
teros_qc2 <- teros_qc1 %>% 
  mutate(value = case_when(research_name == "soil_EC_15cm" & location == "I5" & between(datetime_est, as.POSIXct("2023-01-04 13:00", tz = common_tz), as.POSIXct("2023-05-31 13:00", tz = common_tz)) ~ NA,
                           TRUE ~ value))
```
