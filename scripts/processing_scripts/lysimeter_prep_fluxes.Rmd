...
# 2) Bring in and clean-up lysimeter metadata (volumes)
...
#fill down and up by evacuation date match only, once joined with the pooled volumes: 
  ...
  group_by(plot, grid, evacuation_datetime, collection_datetime, elapsed_time, tz) %>%
...
```{r load vwc}
vwc_l2_15cm = readRDS("~/GitHub/TEMPEST-1-porewater/processed data/TMP_PW_L2_VWC_2022-2023.rds") %>%
  filter(depth == "15") %>%
  mutate(date = as_date(datetime_est)) %>%
  group_by(plot, grid) %>%
   filter(datetime_est < year2_stop & datetime_est > year1_start)

vwc_l2_5cm = readRDS("~/GitHub/TEMPEST-1-porewater/processed data/TMP_PW_L2_VWC_2022-2023.rds") %>%
  filter(depth == "5") %>%
  mutate(date = as_date(datetime_est)) %>%
  group_by(plot, grid) %>%
   filter(datetime_est < year2_stop & datetime_est > year1_start)
...
# 4) filter and summarize VWC by date time lysimeter was collecting
...
vwc_during_collection_15cm = lysimeter_vols %>%
...
vwc_lys_vols = vwc_during_collection_15cm %>%
  right_join(lysimeter_vols, by=c("plot", "grid", "evacuation_datetime", "collection_datetime")) %>%
  distinct()

```
# 5) Load and merge DOC data 
...
doc_l1_pw_grids <- doc_l1_2022 %>%
  ...
  select(plot, grid, evacuation_date, doc_mg_l, tdn_mg_l) %>%
  rename(date = evacuation_date) %>%
...
vwc_lys_vol_doc = vwc_lys_vols %>%
  mutate(date = as_date(evacuation_datetime)) %>%
  full_join(doc_l1_pw_grids, by = c("plot", "grid", "date")) %>%
  drop_na() %>% #TEMPORARY FIX. This is an issue with the merging date column not matching the evacuation dates between the two data frames. still trying to figure out WHY
  select(-date)

write_csv(vwc_lys_vol_doc, "~/GitHub/TEMPEST-1-porewater/processed data/for_fluxes/prelim_Lysimeter_Vols_VWC_DOC_forfluxes.csv")
```
