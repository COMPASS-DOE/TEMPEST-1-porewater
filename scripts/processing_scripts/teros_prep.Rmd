...
teros_qc2_EC <- teros_qc1 %>% 
  mutate(value = case_when(research_name == "soil_EC_5cm" & plot =="Control" & Location_Inventory == "H6" & datetime_est %in% as.POSIXct(c("2022-02-20 22:30", "2022-02-21 09:00", "2022-12-03 13:45", 
                           ...
                           TRUE ~ value)) %>%
    mutate(value = case_when(research_name == "soil_EC_30cm" & plot =="Estuarine-water" & Location_Inventory == "F4" & datetime_est %in% as.POSIXct(c("2023-01-17 03:35"), tz = common_tz)  ~ NA,
                           TRUE ~ value)) %>%
  mutate(value = case_when(research_name == "soil_EC_5cm" & plot =="Estuarine-water" & Location_Inventory == "C3" & datetime_est %in% as.POSIXct(c("2023-07-03 05:10"), tz = common_tz)  ~ NA,
                           TRUE ~ value))
