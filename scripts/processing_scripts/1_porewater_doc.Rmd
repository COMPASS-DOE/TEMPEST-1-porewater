...
,  col_types = c("text","text","text","numeric","text","text","text","text", "text","text","text","text","text")
...
non_event_sample_key_merging <- non_event_sample_key %>%
  ...
  select(Plot, Grid, date, time, evacuation_date, collection_date) 
```
...
pwsite_key_all <- pwsite_key %>% expand(Plot, Grid, date = unique(npoc_wflags_metadata$date)) %>% drop_na()
...
PW_npoc_wflags_metadata_grids_only <- PW_npoc_wflags_metadata %>%
  ...
  select(Event, plot, Grid, datetime, Group, evacuation_date, collection_date, sample_name, doc_mg_l, tdn_mg_l, npoc_flag, tdn_flag)
...
```{r write}
#metadata is too spare in these files so need to clean it up more before saving. 
...
for_fluxes <- PW_npoc_wflags_metadata_grids_only %>%
  ...
  select(Event, plot, grid, sample_name, doc_mg_l, tdn_mg_l, evacuation_date, collection_datetime)

write_csv(for_fluxes, "../TEMPEST-1-porewater/processed data/TMP_PW_GRIDSONLY_NPOC_TDN_L1_May2022-Dec2023.csv")
