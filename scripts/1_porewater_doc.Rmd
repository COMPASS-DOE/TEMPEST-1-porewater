...
#MANUALLY Fix sample names and see if there were multiple of the same sample split for some reason run:
 ...
 all_samples_dilution_corrected2 <- all_samples_dilution_corrected %>% 
  ...
  mutate(sample_name = case_when(sample_name == "TMP_FW_POOL_202310" ~ "TMP_FW_POOL_20231002",
                              sample_name == "TMP_SW_POOL_202310" ~ "TMP_SW_POOL_20231005",
                              TRUE ~ sample_name)) %>%
   ...
   mutate(sample_name = stringr::str_remove(sample_name,"_Subsample")) 
...
PW_npoc_wflags_metadata_pooled_only <- npoc_wflags_metadata %>%
    filter(date < endstudydate & date > startstudydate) %>%
  filter(Grid != "SOURCE") %>%
  filter(Grid != "WELL") %>%
  filter(Grid != "BARGE") %>%
...
source_npoc_wflags_metadata <- npoc_wflags_metadata %>%
  filter(date < endstudydate & date > startstudydate) %>%
...
PW_npoc_wflags_metadata_summarized_noevents <- PW_npoc_wflags_metadata  %>%
  ...
  mutate(
         Group=case_when(lubridate::month(date) != 6 ~ as.character(paste(lubridate::month(date), lubridate::year(date), sep = "-")),
                         lubridate::month(date) == 6 ~ as.character(date))) %>%
  group_by(Plot, Group) %>% 
  summarise(n=n(),
