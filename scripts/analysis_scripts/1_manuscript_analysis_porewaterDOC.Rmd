...
comparison <- doc_l1_pw_grids  %>%
  ...
  mutate(
    ...
    deviation = doc_mg_l - baseline_mean,
...
significant_dates <- comparison %>%
  ...
  select(collection_datetime, doc_mg_l , deviation) %>%
  mutate(year = lubridate::year(collection_datetime)) %>%
  ungroup()
...
tmp1_first_dates <- significant_dates %>% 
  filter(year == "2022") %>%
  filter(collection_datetime == min(collection_datetime))

tmp2_first_dates <- significant_dates %>% 
  filter(year == "2023") %>%
  filter(collection_datetime > WaterDeliveryStop2) %>%
  filter(collection_datetime == min(collection_datetime))

tmp3_first_dates <- significant_dates %>% 
  filter(year > 2023) %>%
  filter(collection_datetime > WaterDeliveryStart3_2024) %>%
  filter(collection_datetime == min(collection_datetime))

tmpdates_doc <- tmp1_first_dates %>%
...
pw_grids_ec <- read_rds("~/GitHub/TEMPEST-1-porewater/processed data/TMP_PW_EC_15cm_2021-2024_L2.rds")
