...
select(plot, grid, evacuation_date, collection_datetime, doc_mg_l) %>%
  mutate( collection_date = lubridate::as_date(collection_datetime),
    ...
    evac_is_month_end = lubridate::day(evacuation_date) == lubridate::days_in_month(evacuation_date),
...
pw_doc_cf <- pw_doc_all %>%
  mutate(logdoc = log(doc_mg_l_mean),
         dateID = interaction(adj_month, year, sep = "_")) %>%
  group_by(plot) %>%
  arrange(dateID, .by_group = TRUE) %>%
