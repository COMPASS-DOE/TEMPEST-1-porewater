...
salt_doc_mean_beforeincrease <- doc_l1_pw_grids %>%
   filter(plot == "Saltwater") %>%
  filter(
        (evacuation_date > EventStart2023 & evacuation_date < "2022-07-18") | # year 1
        (evacuation_date > EventStart2024 & evacuation_date < "2023-08-07") |  # year 2 
        (evacuation_date > endstudydate_yr3 & evacuation_date < "2025-01-28") # year 3
        ) %>%
    summarize(
    sw_mean = mean(doc_mg_l, na.rm = TRUE),
    sw_sd = sd(doc_mg_l, na.rm = TRUE) 
...
cdom_tmp1 <- cdom_allyrs %>%
  rename(datetime = collection_datetime) %>%
  filter(between(datetime, startstudydate_yr1, endstudydate_yr1))

cdom_tmp1_dd <- divide_data(cdom_tmp1, datetime, flood_event_datetime_1, eventB = c(tmpdates_doc1$datetime, tmpdates_doc1$datetime2), eventB_name = "DOC_increase")

cdom_tmp2 <- cdom_allyrs %>%
  rename(datetime = collection_datetime) %>%
  filter(between(datetime, startstudydate_yr2, endstudydate_yr2))

cdom_tmp2_dd <- divide_data(cdom_tmp2, datetime, flood_event_datetime_2, eventB = c(tmpdates_doc2$datetime, tmpdates_doc2$datetime2), eventB_name = "DOC_increase")

cdom_tmp3 <- cdom_allyrs %>%
  rename(datetime = collection_datetime) %>%
  filter(between(datetime, startstudydate_yr3, endstudydate_yr3))

cdom_tmp3_dd <- divide_data(cdom_tmp3, datetime, flood_event_datetime_3, eventB = c(tmpdates_doc3$datetime, tmpdates_doc3$datetime2), eventB_name = "DOC_increase")
...
eems4multi <- cdom_indicies_divided_all %>%
  select(-plot, -timedate_bin, -sample_name, -datetime) 
...
cdom_indicies_divided_all %>%
   ...
   ungroup() %>%
  ggplot(aes(x=factor(timedate_bin, levels = c("Pre","Mid","Btwn Events", "DOC_increase", "Post")), 
               y=A_T_mean)) +
  geom_col() +
