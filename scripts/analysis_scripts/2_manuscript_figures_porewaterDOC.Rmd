...
Figure2a <- pw_doc %>%
  filter(between(evacuation_date, startstudydate_yr1, endplotdate_yr1)) %>%
  ...
  mutate(adj_month = case_when(evacuation_date < WaterDeliveryStart2022 ~ 0,
                               TRUE ~ adj_month)) %>%
  group_by(month) %>%
...
Figure2b <- pw_doc %>%
  filter(between(evacuation_date, endstudydate_yr1, endstudydate)) %>%
  mutate(year = lubridate::year(evacuation_date), 
         month = lubridate::month(evacuation_date),
           # Adjust months: May = 0, June = 1, ..., April = 11
         adj_month = (month(evacuation_date) - 5) %% 12 ) %>%
    mutate(adj_month = case_when(evacuation_date < WaterDeliveryStart1 ~ 0,
                               TRUE ~ adj_month)) %>%
  group_by(month) %>%
  mutate(plot = factor(plot, levels = plot_order),
         Timepoint = cur_group_id(),
  ...
  geom_text(data = data.frame(x = -Inf, y = Inf, label = "  b) Field Year 2"), aes(x, y, label = label), hjust = -0.1, vjust = 1.1, inherit.aes = FALSE)
...
Figure2c <- exp_doc %>%
  mutate(Treatment = factor(Treatment, levels = Treatment_order)) %>%
   ...
   # title("Laboratory Simulation")+
    ...
    theme_classic() +
    theme(legend.position = "bottom") +
  geom_text(data = data.frame(x = -Inf, y = Inf, label = "  c) Laboratory"), aes(x, y, label = label), hjust = -0.1, vjust = 1.1, inherit.aes = FALSE)

print(Figure2c)

Figure2 <- ggarrange(Figure2a, Figure2b, Figure2c, ncol =1)
